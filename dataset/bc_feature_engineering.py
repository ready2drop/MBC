...
def load_data(data_dir : str, 
    ...
    print("--------------Load RawData--------------")
    train_df = pd.read_excel(data_dir, engine='openpyxl')
    
    #Inclusion
    print("--------------Inclusion--------------")
    train_df = train_df[train_df['Inclusion']==1.0]
    print('Total : ', len(train_df))
    
    print("--------------fillNA--------------")
    # data = data.dropna()
    train_df.fillna(0.0,inplace=True)
    print(train_df['Real_stone'].value_counts())
    
    #Column rename
    train_df.rename(columns={'환자번호': 'patient_id','Real_stone':'target' }, inplace=True)
    #column select
    columns = ['patient_id','Duct_diliatation', 'Visible_stone_CT', 'Pancreatitis','target']
    data = train_df[columns]
    
    data['image_name'] = f'{data_dir}/test/' + data['image_name'] + '.jpg'
